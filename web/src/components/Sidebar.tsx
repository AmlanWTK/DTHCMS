...
import { ROUTE_GROUPS, itemForPath, type NavItem, type RouteGroup } from '@/lib/navigation';
 ...
 *
 * Permission is read once, as the active role's permission set, and applied with the pure
 * `can`. The `usePermission` hook is for a single known action at a call site; using it
 * inside a filter would mean calling a hook a variable number of times per render.
 */
export function Sidebar() {
  ...
  const held = user && activeRole !== null ? activePermissions(user, activeRole) : null;
...
function SidebarGroup({
  ...
  const visible: NavItem[] =
    held === null ? [] : group.items.filter((item) => can(held, item.permission));
  ...
  return (
    <div className="app-nav-group">
      ...
      <ul className="app-nav-group__list">
        {visible.map((item) => {
          const current = itemForPath(pathname)?.href === item.href;
          return (
            <li key={item.href}>
              <Link
                href={item.href}
